...
st.write("")
st.info("🔋 OpenChargeMap Nederland API-data wordt per regio geladen (volledige dekking)")
st.markdown("---")
...
def get_all_laadpalen_nederland() -> pd.DataFrame:
    """
    Haalt alle laadpalen van Nederland op via een grid-based benadering.
    Hiermee overschrijden we de max 5000 limiet per request.
    """
    url = "https://api.openchargemap.io/v3/poi/"
    all_data = []
    key = "bbc1c977-6228-42fc-b6af-5e5f71be11a5"

    # Nederland grid (ongeveer 25 km stappen)
    latitudes = np.arange(50.7, 53.6, 0.25)
    longitudes = np.arange(3.3, 7.3, 0.25)

    progress = st.progress(0, text="Laad landelijke data... even geduld aub 🚗⚡")
    ...
    for lat in latitudes:
        for lon in longitudes:
            ...
            try:
                ...
                st.warning(f"⚠ Fout bij ophalen regio ({lat:.2f}, {lon:.2f}): {e}")

            count += 1
            progress.progress(min(count / total_cells, 1.0))

    if not all_data:
        st.error("❌ Geen laadpalen gevonden via OpenChargeMap API.")
        return pd.DataFrame()

    df_all = pd.concat(all_data, ignore_index=True)
    df_all.drop_duplicates(subset="ID", inplace=True)

    # Voeg provincie toe op basis van mapping
    provincie_mapping = {
        "Groningen": "Groningen",
        "Friesland": "Friesland",
        "Fryslân": "Friesland",
    ...
    df_all["Provincie"] = df_all["Provincie"].fillna(df_all["AddressInfo.StateOrProvince"])
    df_all["Provincie"] = df_all["Provincie"].str.strip().str.title()
    st.success(f"🌍 Totaal unieke laadpalen in Nederland: {len(df_all):,}")
...
if page == "⚡️ Laadpalen":
    ...
    with st.spinner(f"🔌 Laad laadpalen voor {provincie_keuze}..."):
        ...
        if provincie_keuze != "Heel Nederland":
            Laadpalen = df_all[df_all["Provincie"].str.contains(provincie_keuze, case=False, na=False)]
        else:
            Laadpalen = df_all  # Heel Nederland = alles
