...
# ------------------- Pagina 2 --------------------------
    ...
    st.markdown("---")
    # (rest van voertuigenpagina blijft ongewijzigd)
...
# ------------------- Pagina 3 --------------------------
    ...
    df_charging1["Datum eerste toelating"] = (
        df_charging1["Datum eerste toelating"].astype(str).str.split(".").str[0]
    )
    df_charging1["Datum eerste toelating"] = pd.to_datetime(
        df_charging1["Datum eerste toelating"], format="%Y%m%d", errors="coerce"
    )
    df_auto_kopie["Datum eerste toelating"] = (
